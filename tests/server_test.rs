...
fn init_log() { LOG_INIT.call_once(|| { env_logger::init(); }); }
...
struct EcProvider { rng: OsRng, priv_key: Vec<u8> }

impl CryptoProvider for EcProvider {
    type CipherSuite = Aes128GcmSha256;
    type Signature = p256::ecdsa::DerSignature;
    fn rng(&mut self) -> impl CryptoRngCore { &mut self.rng }
    fn signer(&mut self) -> Result<(impl signature::SignerMut<Self::Signature>, SignatureScheme), TlsError> {
        use ecdsa::elliptic_curve::SecretKey;
        use p256::ecdsa::SigningKey;
        let sk = SecretKey::from_sec1_der(&self.priv_key).map_err(|_| TlsError::InvalidPrivateKey)?;
...
fn run_blocking_server(
    ...
    std::thread::spawn(move || {
        ...
        let ctx = TlsServerContext::new(&cfg, EcProvider { rng: OsRng, priv_key: key });
...
fn run_async_server(
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            embedded_io_async::Write::flush(&mut tls).await.expect("async flush failed");
...
fn openssl_echo(addr: SocketAddr, extra_args: &[&str], msg: &[u8]) -> (bool, Vec<u8>, String) {
    ...
    let mut args: Vec<String> = vec![
        "s_client".into(), "-connect".into(), conn,
        "-CAfile".into(), ca, "-tls1_3".into(),
        "-verify_return_error".into(), "-quiet".into(),
    ];
    for a in extra_args { args.push(a.to_string()); }

    let mut child = Command::new("openssl")
        .args(&args).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped())
        .spawn().expect("openssl failed to start");
...
fn openssl_connect(addr: SocketAddr, ca: &str, extra_args: &[&str]) -> (bool, String) {
    ...
    let mut args: Vec<String> = vec![
        "s_client".into(), "-connect".into(), conn,
        "-CAfile".into(), ca.to_string(), "-tls1_3".into(),
        "-verify_return_error".into(), "-quiet".into(),
    ];
    for a in extra_args { args.push(a.to_string()); }
    let mut child = Command::new("openssl")
...
fn test_both_inner(
    ...
    for mode in ["blocking", "async"] {
        ...
        client_fn(a);
        if expect_server_ok {
            s.join().unwrap_or_else(|_| panic!("{mode} server panicked"));
        } else {
            s.join().ok();
        }
    }
...
fn test_handshake_and_echo() {
    init_log();
    test_both(|c| c, |addr| {
        let (ok, resp, se) = openssl_echo(addr, &["-ciphersuites", "TLS_AES_128_GCM_SHA256"], b"hello\n");
        assert!(ok, "failed.\nstderr: {se}");
...
fn test_protocol_and_cipher() {
    init_log();
    test_both(|c| c, |addr| {
        let (ok, _resp, se) = openssl_echo(addr, &["-ciphersuites", "TLS_AES_128_GCM_SHA256"], b"cipher check\n");
        assert!(ok, "failed.\nstderr: {se}");
        // -quiet still shows verify info on stderr
        assert!(se.contains("TLSv1.3") || se.contains("verify return:1"), "no verification in:\n{se}");
    });
}
...
fn test_wrong_ca_rejected() {
    init_log();
    let wrong_ca = data_dir().join("rsa-ca-cert.pem").to_str().unwrap().to_string();
    test_both_server_may_fail(|c| c, |addr| {
            ...
            std::thread::spawn(move || {
                ...
                while all.len() < 8000 {
                    match ER::read(&mut tls, &mut buf) {
                        Ok(0) => break, Ok(n) => all.extend_from_slice(&buf[..n]),
                        Err(e) => panic!("read: {e:?}"),
                    }
                }
                let mut w = 0;
                while w < all.len() { w += EW::write(&mut tls, &all[w..]).unwrap(); }
...
fn test_hrr() {
    ...
    test_both(|c| c, |addr| {
        ...
        let (ok, resp, se) = openssl_echo(addr, &["-ciphersuites", "TLS_AES_128_GCM_SHA256"], b"hrr\n");
...
fn test_x25519_direct() {
    ...
    test_both(|c| c, |addr| {
        let (ok, resp, se) = openssl_echo(
            addr, &["-ciphersuites", "TLS_AES_128_GCM_SHA256", "-groups", "X25519"], b"x25519\n",
        );
...
fn test_client_cert_auth() {
    init_log();
    let cert = data_dir().join("client-cert.pem").to_str().unwrap().to_string();
    let key = data_dir().join("client-key.pem").to_str().unwrap().to_string();
    test_both(
...
fn test_alpn() {
    ...
    test_both(
        ...
        |addr| {
            ...
            assert!(String::from_utf8_lossy(&resp).contains("alpn test"), "no echo");
        },
    );
}
...
fn test_close_notify() {
    init_log();
    test_both(|c| c, |addr| {
        let (ok, resp, se) = openssl_echo(addr, &["-ciphersuites", "TLS_AES_128_GCM_SHA256"], b"close\n");
        assert!(ok, "should exit cleanly.\nstderr: {se}");
        assert!(String::from_utf8_lossy(&resp).contains("close"));
        assert!(!se.contains("unexpected eof"), "missing close_notify.\nstderr: {se}");
    });
}
...
fn test_unknown_named_groups_ignored() {
    ...
    test_both(|c| c, |addr| {
        let (ok, resp, se) = openssl_echo(
            addr, &["-groups", "ffdhe2048:P-256"], b"unknown groups\n",
        );
        assert!(ok, "should ignore unknown group and use P-256.\nstderr: {se}");
...
fn test_hrr_unsupported_group_only() {
    ...
    test_both(|c| c, |addr| {
        let (ok, resp, se) = openssl_echo(
            addr, &["-groups", "P-384:P-256"], b"hrr p384\n",
        );
